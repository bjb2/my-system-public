...
│   ├── domains/       # Domain research & synthesis (paid media, AI, etc.)
│   ├── tools/         # Implementation patterns, gotchas (tag #gotcha vs #research)
│   ├── system/        # Org system design patterns
...
tags: []
blocked-by: []
first-action: null       # one physical action, under 2 min, no prior decisions required
acceptance-criteria: null  # what runtime behavior confirms the task is done
---
```
...
**first-action field:** A single concrete physical action that starts the task — specific enough that no decisions are needed before doing it. Examples: `"Open maps.google.com, search X"`, `"Open file Y, read lines 1-20"`. When `null`, the task has no explicit entry point defined.

**acceptance-criteria field:** What you would observe at runtime that proves the feature works. Used by the QA reviewer agent to generate a focused verification checklist. Write it as observable behavior: `"Dragging a file moves it without an error toast"`, `"Clipboard paste into terminal shows the copied text"`. When `null`, the QA agent will infer criteria from the task body.

**When Claude creates a task with abstract first steps** (e.g., "identify...", "research...", "figure out..."), populate `first-action:` automatically with the most concrete possible first move — a URL to open, a file to read, a search to run.
...
### Decision → Execute Flow

When processing a decision where **all open questions are resolved** (either by the user or self-evidently):

1. Resolve the decision in-place (update frontmatter `status: complete`, replace "Decision Needed" section with "Decision: [what was decided]")
2. Create the task immediately
3. **Execute the task immediately** — do not pause to confirm

Only stop and confirm if open questions remain unanswered. If the decision file itself answers everything, treat the user's direction as authorization to go all the way through.
