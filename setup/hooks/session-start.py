...
def find_org_dir(data: dict) -> str | None:
    """
    Detect the org root directory from multiple sources:
    1. ORG_DIR env var (explicit override)
    2. cwd field from hook data (Claude Code passes this on SessionStart)
    3. Current working directory if it has CLAUDE.md
    """
    ...
    if os.path.isfile(os.path.join(process_cwd, "CLAUDE.md")):
        return process_cwd
